...
Shipped work is recorded in `docs/PHASE*.md` (currently `PHASE1.md`–`PHASE4.md` plus `PHASE3-enhance-tests.md` and `PHASE4-followup.md`). Read those before re-deriving status from code or chat. The week plan is `docs/project-plan.md`. Planned Phase 5 (Reliability + HITL) has not started.
...
- **`harness_version` ≠ sandbox.** `v0` = ReAct, `v1` = LangGraph Plan-Execute, `v2` = V1 + hybrid retrieve + `search_code`. Docker (`issue-pilot-sandbox:py312`) is shared runtime. Failed verify goes diagnose → execute while `retry_count < MAX_RETRY` (2 diagnose cycles = 1 retry), then `needs_human`.
- **Ablation.** `python cli.py compare <id>` still runs V0 then V1 only. V2 is opt-in: `python cli.py solve <id> --harness v2`. Prefer hard split `issue-008`–`011`. Smoke is `issue-001`–`007`. Gold pass/fail is not the RAG metric; use `recall_at_5` / `python cli.py retrieve`. Grep ablation ranks `app/**/*.py` only (same corpus as the indexer); agent `grep_code` still searches the whole repo. Live V2 defaults to hashing + issue-only query; `retrieve` defaults to fastembed. Aggregate with `python cli.py report` (cohorts by `base_commit` / model / temperature / sandbox image).
